...
namespace Valkyrja.modules
{
	public class Karma: IModule
	{
		...
		private readonly Regex RegexKarma = new Regex(".*(?<!(\\\\|\\bno))(thank(?!sgiving)|thx|ʞuɐɥʇ|dankj?e(wel)?|vielen dank|gracias|merci(?!al)|grazie|arigato|dziękuję|dziekuje|obrigad|(?:^|\\W)ty(sm)?(?:$|\\W)).*", RegexOptions.Compiled | RegexOptions.IgnoreCase);
