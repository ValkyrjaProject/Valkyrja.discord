...
namespace Valkyrja.modules
{
	public class Karma: IModule
	{
		...
		private readonly Regex RegexKarma = new Regex(".*(?<!(\\\\|(^|\\W)no ))(thank(?!sgiving)|thx|cheers|ʞuɐɥʇ|dankj?e(wel)?|vielen dank|gracias|merci(?!al)|grazie|arigato|dziękuję|dziekuje|obrigad|(?:^|\\W)ty+(sm)?(?:$|\\W)).*", RegexOptions.Compiled | RegexOptions.IgnoreCase);
