...
namespace Valkyrja.modules
{
	public class Karma: IModule
	{
		...
		private readonly Regex RegexKarma = new Regex(".*(?<!\\\\)(thank(?!sgiving)|thx|ʞuɐɥʇ|danke|vielen dank|gracias|merci(?!al)|grazie|arigato|dziękuję|dziekuje|obrigad|(?:^|\W)ty(sm)?(?:$|\W)).*", RegexOptions.Compiled | RegexOptions.IgnoreCase);
